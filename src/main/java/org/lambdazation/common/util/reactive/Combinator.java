...
public interface Combinator {
	static <A> Flow<Event<A>> increment(A a, Event<Function<A, A>> event) {
		...
		return
			Flow.<A, Event<A>> bfix(
				b ->
				Flow.retrieve(b.fmap(Functional.reverse()), event).compose(
				e ->
				Flow.store(a, e).fmap(Product.ofProductRight(e))));
		// @formatter:on
	}
	...
	static <A> Flow<Behavior<A>> accumulate(A a, Event<Function<A, A>> event) {
		// @formatter:off
		return
			Flow.<A, Behavior<A>> bfix(
				b ->
				Flow.retrieve(b.fmap(Functional.reverse()), event).compose(
				e ->
				Flow.store(a, e).fmap(Product.ofProductBoth())));
		// @formatter:on
	}
