...
import org.lambdazation.client.gui.widget.WidgetWrapper;
...
public final class GuiTest extends GuiScreen {
	...
	public GuiTest(Lambdazation lambdazation) {
		...
		WidgetWrapper<ModelBase, ViewWrapper<ModelBase>> widgetWrapper = new WidgetWrapper<>(
			new ModelBase(),
			new ViewWrapper<>(widgetButton2));
		...
		WidgetContainer<ModelBase, ViewContainer<ModelBase>> widgetContainer = new WidgetContainer<>(
			...
			ValueBuilder.build(new ViewContainer<>(), viewContainer -> {
				...
				viewContainer.addComponent(widgetWrapper, 10.0D, 40.0D);
